...
uint16 SimpleBLEPeripheral_ProcessEvent( uint8 task_id, uint16 events )
{
    ...
    if ( events & SBP_NRF_PERIODIC_TX_EVT )
    {
        if(nrf_tx_cnt)
        {
            extern uint8_t phy_rf_send_data(uint8_t* din, uint8_t dLen);
            advertData[2]=nrf_tx_cnt;
            advertData[1]=nrf_tx_intv;
            uint8_t ret = phy_rf_send_data(advertData,32);
...
//        LOG_DEBUG("[PHY] %02d %02d %d\n",allPhy,txPhy,status);
        ...
        #endif
        //[0x10 a1 a2] :set smart rf trx
        else if(newValue[0]==0x10)
        {
            nrf_tx_intv = newValue[1];
            nrf_tx_cnt = newValue[2];
            LOG("[PPP TX] Intv%04d Cnt%04d\n", nrf_tx_intv,nrf_tx_cnt);
            osal_set_event( simpleBLEPeripheral_TaskID, SBP_NRF_PERIODIC_TX_EVT);
        }
        //[0x11 a1 a2] :set smart rf rx
        else if(newValue[0]==0x11)
        {
            nrf_rx_intv = newValue[1];
            nrf_rx_cnt = newValue[2];
            LOG("[PPP RX] Intv%04d Cnt%04d\n", nrf_rx_intv,nrf_rx_cnt);
            osal_set_event( simpleBLEPeripheral_TaskID, SBP_NRF_PERIODIC_RX_EVT);
        }
        else if(newValue[0]==0x12)
        {
            ...
            if(newValue[1]==0x01)
            {
                ret= phy_rf_start_rx(RFPHY_RX_SCAN_ALLWAYS_ON);

                if(ret == PPlus_SUCCESS)
                {
                    LOG("[PPP RX] Start AllwaysON  ret %d\n", ret);
                }
            }
            else
            {
