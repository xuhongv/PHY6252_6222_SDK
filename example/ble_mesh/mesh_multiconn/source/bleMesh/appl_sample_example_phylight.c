...
extern UCHAR blebrr_prov_started;
    ...
    extern uint16_t proxy_conn_hndl ;
    extern uint16_t private_conn_hndl ;
    extern uint16_t prov_conn_hndl ;
#endif
...
DECL_STATIC UCHAR blebrr_prov_reset = MS_FALSE;
...
void UI_proxy_start_adv(MS_SUBNET_HANDLE subnet_handle, UCHAR proxy_adv_mode)
{
    ...
    if(proxy_adv_mode == MS_PROXY_NODE_ID_ADV_MODE)
    {
        blebrr_gatt_mode_set(BLEBRR_GATT_PROXY_MODE);
//      blebrr_set_gattmode_pl (BLEBRR_GATT_PROXY_MODE);
    ...
    CONSOLE_OUT("Start Proxy Advertisements with %s for Subnet 0x%04X\n",
                (proxy_adv_mode == MS_PROXY_NODE_ID_ADV_MODE) ? "Node Identity" : "Private adv",
                subnet_handle);
...
API_RESULT UI_set_brr_scan_rsp_data (void)
{
    ...
    */
    ...
    {
        0x0D, 0x09, 'P', 'H', 'Y', '-', 'M', 'S', 'H', 'M', 'U', 'L', 'T', 'I'
    };
...
)
{
    ...
    case BLEBRR_GATT_IFACE_DOWN:
        ...
        #else
        uint16_t  conn_hndl ;
        conn_hndl = (uint16_t)ev_param;
        CONSOLE_OUT("\r\n >> prov_conn_hndl %x,proxy_conn_hndl %x,private_conn_hndl %x,gatt mode %x\r\n",prov_conn_hndl,proxy_conn_hndl,private_conn_hndl,blebrr_gatt_mode_get());

        if(prov_conn_hndl == conn_hndl)
        {
            if(BLEBRR_GATT_PROV_MODE == blebrr_gatt_mode_get())
            {
                CONSOLE_OUT("start prov\n");
                UI_setup_prov(PROV_ROLE_DEVICE, PROV_BRR_ADV|PROV_BRR_GATT);
            }
            else
            {
                if(thandle == EM_TIMER_HANDLE_INIT_VAL)
            ...
            prov_conn_hndl = 0xFFFF;
        }
        else if(proxy_conn_hndl == conn_hndl)
        {
            if(blebrr_prov_reset)
            {
                if(thandle == EM_TIMER_HANDLE_INIT_VAL)
                {
                    EM_start_timer (&thandle, 3, timeout_cb, NULL, 0);
                ...
                blebrr_prov_reset = MS_FALSE;
            }
            else
            {
                CONSOLE_OUT("start proxy\n");
                UI_proxy_start_adv(0x0000, MS_PROXY_NODE_ID_ADV_MODE);
            }

            proxy_conn_hndl = 0xFFFF;
        }
        else if(private_conn_hndl == conn_hndl)
        {
            CONSOLE_OUT("start private\n");
            UI_proxy_start_adv(0x0000, MS_PROXY_PRIV_ADV_MODE);
            private_conn_hndl = 0xFFFF;
        }
        else
        {
            //exception case
